...
$source = Join-Path $paperDir "reference-relative-believability-peer.md"
$css = Join-Path $paperDir "peer-review.css"
$html = Join-Path $paperDir "reference-relative-believability-peer.html"
$pdf = Join-Path $paperDir "reference-relative-believability-peer.pdf"
$arxivDir = Join-Path $paperDir "arxiv"
$arxivTex = Join-Path $arxivDir "reference-relative-believability-peer.tex"
...
$arxivMd = Join-Path $arxivDir "_reference-relative-believability-peer-arxiv.md"
...
if (-not $chrome) {
  throw "Chrome or Edge is required to print the peer-review PDF."
}
...
pandoc $source `
  ...
  --css $css `
  --metadata pagetitle="Reference-relative believability - peer review manuscript" `
  --output $html
